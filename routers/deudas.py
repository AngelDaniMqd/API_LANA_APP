...
def listar_deudas(
    ...
    """
    Lista SOLO las deudas del usuario autenticado.
    """
    return (
        db.query(Deuda)
        .filter(Deuda.usuarios_id == current_user.id)
        .order_by(Deuda.fecha_inicio.desc())
        .all()
    )
...
def crear_deuda(
    ...
    monto: float = Form(..., description="Monto de la deuda"),  # DECIMAL en BD
    ...
    """
    Crea deuda para el usuario autenticado.
    """
    categoria_metodo = (
        db.query(CategoriaMetodo)
        .filter(CategoriaMetodo.id == categori_metodos_id)
        .first()
    )
    ...
    db_deuda = Deuda(
        ...
        usuarios_id=current_user.id,  # <<<<<< AMARRA AL USUARIO
    )
    db.add(db_deuda)
    db.commit()
    db.refresh(db_deuda)
...
def actualizar_deuda(
    ...
    monto: Optional[float] = Form(None, description="Nuevo monto"),  # DECIMAL en BD
    ...
    """
    Actualiza SOLO si la deuda pertenece al usuario autenticado.
    """
    deuda = (
        db.query(Deuda)
        .filter(Deuda.id == deuda_id, Deuda.usuarios_id == current_user.id)
        .first()
    )
    if not deuda:
...
def eliminar_deuda(
    ...
    """
    Elimina SOLO si la deuda pertenece al usuario autenticado.
    """
    deuda = (
        db.query(Deuda)
        .filter(Deuda.id == deuda_id, Deuda.usuarios_id == current_user.id)
